...
export default function ArrowDeviceDetail(
  { deviceId }: ArrowDeviceDetailProps,
) {
  ...
  useEffect(() => {
    ...
    const handleAssignZone = async () => {
      ...
      try {
        ...
        state.assignError = err instanceof Error
          ? err.message
    ...
    const template = html`
      <div class="p-8 max-w-5xl mx-auto">
        ...
        ${() =>
          state.loading
            ? html`
              <div class="text-center py-12 text-stone-400">Loading device...</div>
            `
            : html`
          ...
          return html`
              ...
              >
                <div>
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="text-sm font-mono text-stone-400">${device
                      .deviceCode}</span>
                  </div>
                ...
                ${device.status !== 4
                  ? html`
                    <button
                      type="button"
                      @click="${() => {
                        state.showDecommissionDialog = true;
                      }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              ...
              <div class="bg-white rounded-2xl border border-stone-200 shadow-sm p-6">
                <h3
                  class="text-lg font-bold text-stone-800 mb-4 border-b border-stone-100 pb-2"
                >
                  Zone Assignment
                </h3>
                <div class="space-y-4">
                    ...
                    >
                      ...
                      ${state.zones.map((z: ZoneSummaryDto) =>
                        html`
                          <option value="${z
                            .id}" selected="${z.id ===
                            state.selectedZoneId}">${z.name}</option>
                        `
                      )}
                    </select>
                    <button
                      ...
                      disabled="${() =>
                        state.assigning || !state.selectedZoneId}"
              ...
              <div class="bg-white rounded-2xl border border-stone-200 shadow-sm p-6">
                ...
                <div class="space-y-3">
                  <div class="grid grid-cols-3 gap-4 border-b border-stone-100 py-2">
                    ...
                    <span class="col-span-2 text-stone-800 text-sm">${sensorTypeName(
                      device.sensorType,
                    )}</span>
                  </div>
                  <div class="grid grid-cols-3 gap-4 border-b border-stone-100 py-2">
                    <span class="text-stone-500 text-sm font-medium">Device ID</span>
                    <span class="col-span-2 text-stone-800 font-mono text-sm"
                    >${device.id}</span>
                  </div>
                  ${device.gridPos
                    ? html`
                      <div class="grid grid-cols-3 gap-4 border-b border-stone-100 py-2">
                        ...
                        <span class="col-span-2 text-stone-800 font-mono text-sm"
                        >(${device.gridPos.x}, ${device.gridPos.y}, ${device
                          .gridPos.z})</span>
                      </div>
                    `
                    : html`
                    ...
                    ? html`
                      <div class="grid grid-cols-3 gap-4 border-b border-stone-100 py-2">
                        ...
                        <span class="col-span-2 text-stone-800 font-mono text-sm"
                        >${device.geoPos.latitude.toFixed(6)}, ${device.geoPos
                ...
                <div class="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 mb-8">
                  ...
                  <div class="bg-stone-50 rounded-lg p-4 border border-stone-100">
                    ${Object.entries(device.metadata).map(([k, v]) =>
                      html`
                        <div
                          class="flex justify-between py-2 border-b border-stone-100 last:border-0 text-sm"
                        >
