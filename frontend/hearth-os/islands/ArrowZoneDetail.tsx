...
import type { ZoneDetailDto, DeviceSummaryDto } from "../utils/farmos-client.ts";
  ...
  useEffect(() => {
    ...
    const fetchZone = async () => {
      ...
      try {
        ...
        state.error = err instanceof Error ? err.message : "Failed to load zone";
      } finally {
        state.loading = false;
      }
    };
    ...
    const handleArchive = async () => {
      ...
      try {
        const { IoTAPI } = await import("../utils/farmos-client.ts");
        await IoTAPI.archiveZone(zoneId, { zoneId, reason: "Archived by user" });
        globalThis.location.href = "/iot/zones";
      } catch (err: unknown) {
        state.error = err instanceof Error ? err.message : "Failed to archive zone";
      ...
      return html`
        ...
        >
          <div class="flex items-center justify-between mb-2">
            <h4 class="font-bold text-stone-800 text-sm truncate">${device.name}</h4>
    ...
    const template = html`
      <div class="p-8 max-w-5xl mx-auto">
        <!-- Back link -->
        <div class="mb-6">
          <a href="/iot/zones" class="text-emerald-600 hover:text-emerald-800 font-semibold inline-block transition">
            &larr; Back to Zones
          </a>
        </div>
        ...
        <!-- Loading -->
        ${() => state.loading
          ? html`<div class="text-center py-12 text-stone-400">Loading zone...</div>`
          : html``}

        <!-- Zone detail -->
        ${() => {
          if (state.loading || !state.zone) return html``;
          ...
          return html`
              ...
              <div class="p-8 border-b border-stone-100 flex flex-col md:flex-row justify-between md:items-center gap-6 bg-gradient-to-br from-stone-50 to-white">
                <div>
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    ${zone.isArchived
                      ? html`<span class="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border bg-stone-200 text-stone-500 border-stone-300">Archived</span>`
                      : html``}
                  </div>
                  <h1 class="text-3xl font-bold text-stone-800 tracking-tight">${zone.name}</h1>
                  ${zone.description
                    ? html`<p class="text-stone-500 mt-1">${zone.description}</p>`
                    : html``}
                </div>
                ${!zone.isArchived
                  ? html`
                    <button
                      type="button"
                      @click="${() => { state.showArchiveDialog = true; }}"
            ...
            ${zone.devices && zone.devices.length > 0
              ...
              : html`
                <div class="bg-stone-50 border border-stone-200 rounded-2xl p-12 text-center">
                  ...
                  <p class="text-sm text-stone-500">Assign devices from the device detail page.</p>
                </div>
              `}
            ...
            ${ArrowConfirmDialog({
              ...
              title: "Archive Zone",
              message: "Are you sure you want to archive this zone? Devices will be unassigned.",
              confirmLabel: "Archive",
              danger: true,
              onConfirm: handleArchive,
              onCancel: () => { state.showArchiveDialog = false; },
            })}
          `;
        }}
      </div>
