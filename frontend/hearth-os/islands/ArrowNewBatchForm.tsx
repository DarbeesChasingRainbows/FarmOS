  ...
  useEffect(() => {
    ...
    const onSubmit = async (e: Event) => {
      ...
      try {
        ...
        if (state.batchType === "sourdough") {
          await HearthAPI.startSourdough({
            ...
            ingredients: [
              {
                name: "Bread Flour",
                amount: { value: 1000, unit: "grams", type: "weight" },
              },
              {
                name: "Water",
                amount: { value: 750, unit: "grams", type: "weight" },
              },
              {
                name: "Salt",
                amount: { value: 20, unit: "grams", type: "weight" },
              },
            ],
          });
          ...
          await HearthAPI.startSourdough({
            ...
            ingredients: [
              {
                name: bt?.label ?? state.batchType,
                amount: { value: 1, unit: "batch", type: "count" },
              },
            ],
          });
        }
        ...
        showToast(
          "success",
          "Batch started!",
          `${bt?.label ?? state.batchType} batch "${code}" is now active.`,
        );
        closeModal();
      } catch (err: unknown) {
        showToast(
    ...
    const inputClass = (field: string) => () =>
      `w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition text-sm ${
        (state.errors as Record<string, string>)[field]
      ...
      <div>
        ...
        ${() => {
          ...
          return html`
            ...
            >
              <div
                class="bg-white rounded-xl shadow-xl w-full max-w-xl mx-4 overflow-hidden animate-[scaleIn_0.2s_ease-out]"
              >
                <!-- Header -->
                <div
                ...
                >
                  ...
                  <button
                    @click="${closeModal}"
                    class="text-stone-400 hover:text-stone-600 hover:bg-stone-200 rounded-full p-1.5 transition"
                  >
                    ✕
                  </button>
                </div>
                ...
                <div class="p-6">
                  ...
                  <p class="text-sm font-semibold text-stone-700 mb-3">
                    ...
                    ${batchTypes.map((bt) =>
                      html`
                        <button
                          ...
                          class="${() =>
                            `flex flex-col items-center gap-1 py-3 px-2 rounded-xl border text-sm font-medium transition ${
                              state.batchType === bt.id
                  ...
                  <form @submit="${onSubmit}" class="flex flex-col gap-4">
                    ${ArrowFormField({
                      ...
                      children: html`
                        <textarea
                          class="${() =>
                            `w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition text-sm resize-none ${
                              (state.errors as Record<string, string>).notes
                                ? "border-red-400 bg-red-50"
                                : "border-stone-300"
                            }`}"
                          rows="2"
                    ...
                    <div class="flex justify-end gap-3 mt-2">
                      ...
                      >
                        ${() => {
                          const bt = batchTypes.find((t) =>
                            t.id === state.batchType
                          );
                          return state.isSubmitting
                            ? "Starting..."
                            : `Start ${bt?.label ?? "Batch"}`;
                        }}
                      </button>
                    </div>
                  </form>
