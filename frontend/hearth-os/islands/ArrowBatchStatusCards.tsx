...
const phaseDescriptions: Record<string, string> = {
  ...
  Proofing:
    "Shaped dough rising in banneton. Ready when poke test springs back slowly.",
...
export default function ArrowBatchStatusCards(
  { initialBatches }: ArrowBatchStatusCardsProps,
) {
  ...
  useEffect(() => {
      ...
      html`
        ...
        >
          <div class="flex items-center justify-between mb-2">
            ...
            <span
              class="text-[10px] uppercase tracking-widest font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full"
            >
              ${() => batch.phase}
            </span>
          </div>
          <h3 class="font-bold text-stone-800 text-sm">${batch.code}</h3>
          <p class="text-xs text-stone-400 mt-0.5">Started ${batch
            .startedAt}</p>
    ...
    const expandedPanel = () => {
      const selectedBatch = state.batches.find((b: Batch) =>
      ...
      return html`
        <div
          class="mt-6 bg-white rounded-3xl border border-amber-200/50 shadow-2xl shadow-amber-900/5 p-6 md:p-8 animate-[scaleIn_0.2s_ease-out] relative overflow-hidden backdrop-blur-md"
        >
          <div
            class="absolute -right-32 -top-32 w-64 h-64 bg-linear-to-bl from-amber-200 to-transparent rounded-full blur-3xl opacity-30"
          >
          </div>
          ...
          <div class="flex items-start justify-between mb-4 relative z-10">
            <div>
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-lg font-bold text-stone-800">${selectedBatch
                  .code}</h3>
                <span
                  class="text-[10px] font-bold text-amber-600 bg-amber-100 px-2 py-1 rounded-full animate-pulse ml-2 tracking-widest uppercase"
                >Arrow.js</span>
              </div>
          ...
          <div
            class="bg-amber-50 rounded-lg p-4 mb-4 border border-amber-100 relative z-10"
            title="${phaseDescriptions[selectedBatch.phase] || "Current Phase"}"
          >
            <div class="flex items-center gap-2 mb-1">
              <span class="text-sm font-semibold text-amber-800"
              >Phase: ${() => selectedBatch.phase}</span>
            </div>
            <p class="text-xs text-amber-700">${() =>
              phaseDescriptions[selectedBatch.phase]}</p>
          </div>

          <div class="grid grid-cols-3 gap-4 mb-4 relative z-10">
            <div
              class="bg-stone-50 rounded-lg p-3 text-center border border-stone-100/50 shadow-sm"
            >
              <p class="text-2xl font-mono font-bold ${() =>
                phColor(selectedBatch.ph)}">${() => selectedBatch.ph}</p>
          ...
          <div
            class="flex items-center gap-3 pt-4 border-t border-stone-100 relative z-10"
          >
            <button
              type="button"
              @click="${() => handleAdvance(selectedBatch)}"
              class="${() =>
                state.advancingId === selectedBatch.id
                  ? "opacity-50"
                  : ""} px-4 py-2 text-xs font-bold rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition shadow-sm"
            >
              ⏭ ${() =>
                state.advancingId === selectedBatch.id
                  ? "Advancing..."
            ...
            <a
              href="/batches"
              class="px-4 py-2 text-xs font-bold rounded-lg bg-stone-100 text-stone-600 hover:bg-stone-200 transition"
            >
              View All Batches →
            </a>
          </div>
