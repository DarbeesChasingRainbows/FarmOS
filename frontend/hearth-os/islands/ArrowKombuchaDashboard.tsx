  ...
  useEffect(() => {
    ...
    const batchCard = (batch: ActiveFermentationMonitorDto) => {
      const borderClass = batch.isSafe
        ? "border-emerald-200"
        : "border-red-300";
      const phDisplay = batch.currentPH !== null
        ? batch.currentPH.toFixed(2)
        : "\u2014";
      const dropDisplay = batch.dropRatePerHour !== null
      ...
      return html`
        ...
        >
          <div class="flex items-center justify-between mb-3">
            ...
            <span
              class="text-xs font-semibold px-2 py-0.5 rounded-full ${batch
                  .isSafe
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-xs text-stone-400 uppercase tracking-wider">Phase</span>
              <span class="text-sm font-medium text-stone-700">${batch
                .phase}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs text-stone-400 uppercase tracking-wider">pH</span>
              <span class="text-sm font-bold ${phColor(
                batch.currentPH,
              )}">${phDisplay}</span>
            </div>
            <div class="flex items-center justify-between">
              ...
              >Drop Rate</span>
              <span class="text-sm font-medium text-stone-600">${dropDisplay}</span>
            </div>
          </div>
          ${batch.statusMessage
            ? html`
                ...
                ${() =>
                  filteredBatches().length === 0
                    ...
                    : html`
                      ...
                      >
                        ${() =>
                          filteredBatches().map((batch) => batchCard(batch))}
                      </div>
                    `}
