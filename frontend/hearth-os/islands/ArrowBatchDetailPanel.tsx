...
const phaseVariant = (phase: string) => {
  ...
  if (["Proofing", "Secondary", "Aging", "Bottling"].includes(phase)) {
...
export default function ArrowBatchDetailPanel(
  { initialBatches }: ArrowBatchDetailPanelProps,
) {
  ...
  useEffect(() => {
    ...
    const getSelected = () =>
      state.batches.find((b) => b.id === state.selectedId) ?? null;
    ...
    const handleComplete = async (batch: Batch) => {
      ...
      try {
        ...
        if (batch.type === "sourdough") {
          ...
          await HearthAPI.completeSourdough(batch.id, {
            yieldQty: { value: 1, unit: "batch", type: "count" },
          });
        }
        showToast(
          "success",
    ...
    const renderBatchCard = (batch: Batch) => {
      ...
      return html`
        <button
          ...
          class="${() =>
            `bg-white rounded-xl border shadow-sm p-5 hover:shadow-md transition text-left w-full cursor-pointer ${
              isSelected()
        ...
        >
          ...
          <div
            class="flex items-center justify-between mt-4 pt-3 border-t border-stone-100"
          >
            <div>
      ...
      <div class="relative flex min-h-[500px]">
        ...
        <div class="${() =>
          ...
          ${() => {
            if (state.batches.length === 0) {
              return html`
                <div class="flex flex-col items-center justify-center py-20 px-8">
                  <div
                    class="w-24 h-24 bg-amber-50 rounded-full flex items-center justify-center mb-6 border-2 border-amber-100"
                  >
                  ...
                  <p class="text-stone-500 text-center max-w-md mb-8 leading-relaxed">
                    Start tracking your fermentations — sourdough, kombucha, kimchi, sauerkraut,
                    jun, or anything else. Begin by clicking "+ New Batch" above.
                  </p>
                  <div
                    class="flex flex-wrap items-center justify-center gap-4 text-sm text-stone-400"
                  >
            ...
            return html`
              ${types.map((type) => {
                const typeBatches = state.batches.filter((b) =>
                  b.type === type
                );
        ...
        <!-- Slide-out Sidebar -->
        <aside class="${() =>
          `fixed top-0 right-0 h-full w-[400px] bg-white border-l border-stone-200 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out overflow-y-auto ${
          ...
          ${() => {
            ...
            return html`
                ...
                >
                  <div>
                    <div class="flex items-center gap-2 mb-2">
                      ...
                      <h3 class="text-2xl font-bold text-stone-800">${batch
                        .code}</h3>
                    </div>
                ...
                <div class="flex-1">
                  ...
                  <div class="bg-amber-50 rounded-lg p-4 mb-5 border border-amber-100">
                    <div class="flex items-center gap-2 mb-1">
                      ...
                      >Current Phase: ${batch.phase}</span>
                      ${ArrowTooltip({
                        text:
                          "Each batch progresses through phases. Advance when conditions are met.",
                        children: ArrowInfoIcon(),
                      })}
                    </div>
                    <p class="text-xs text-amber-700 leading-relaxed">${phaseDescriptions[
                      batch.phase
                    ] ?? "Fermentation is in progress."}</p>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-3 mb-6">
                    ...
                    >
                      ...
                      <div class="flex items-center justify-center gap-1 mt-1">
                        ...
                        ${ArrowTooltip({
                          text:
                            "Fermentation lowers pH. Target depends on product type.",
                          position: "top",
                          children: ArrowInfoIcon(),
                        })}
                      </div>
                    ...
                    <div
                      class="bg-stone-50 rounded-xl p-4 text-center border border-stone-100"
                    >
                      <p class="text-lg font-mono font-bold text-stone-700">${batch
                        .startedAt.split(",")[0]}</p>
                      <p class="text-xs text-stone-500 uppercase tracking-wide mt-1">
                        Started
                      </p>
                ...
                state.confirmAdvance
                  ? html`
                    <div
                      ...
                      @click="${() => state.confirmAdvance = false}"
                    >
                      ...
                      >
                        <h3 class="text-lg font-bold text-stone-800 mb-2">Advance ${batch
                          .code}?</h3>
                        <p class="text-sm text-stone-600 mb-6 leading-relaxed">
                          Move "${batch.code}" from ${batch
                            .phase} to the next phase. Make sure
                          conditions are met before advancing.
                        </p>
                        <div class="flex gap-3 justify-end">
                          ...
                          <button
                            @click="${() => handleAdvance(batch)}"
                            class="px-4 py-2 text-sm font-semibold bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition shadow-sm"
                          >
                            Advance Phase
                          </button>
                        </div>
              ...
              ${() =>
                state.confirmComplete
                  ? html`
                    <div
                      class="fixed inset-0 z-50 flex items-center justify-center"
                      @click="${() => state.confirmComplete = false}"
                    >
                      <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
                      <div
                        class="relative bg-white rounded-xl shadow-2xl border border-stone-200 p-6 max-w-md w-full mx-4 animate-[scaleIn_0.2s_ease-out]"
                        @click="${(e: Event) => e.stopPropagation()}"
                      >
                        <h3 class="text-lg font-bold text-stone-800 mb-2">Complete ${batch
                          .code}?</h3>
                        <p class="text-sm text-stone-600 mb-6 leading-relaxed">
                          Mark "${batch
                            .code}" as complete. This finalizes the batch and records the
                          yield.
                        </p>
                        <div class="flex gap-3 justify-end">
                          <button
                          ...
                          <button
                            @click="${() => handleComplete(batch)}"
                            class="px-4 py-2 text-sm font-semibold bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition shadow-sm"
                          >
                            Complete Batch
                          </button>
                        </div>
