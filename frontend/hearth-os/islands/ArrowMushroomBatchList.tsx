...
const formatPhase = (phase: number) => {
  const phases: Record<number, { label: string; color: string }> = {
    0: { label: "Incubating", color: "bg-stone-100 text-stone-800 border-stone-200" },
    1: { label: "Pinning", color: "bg-blue-50 text-blue-800 border-blue-200" },
    2: { label: "Fruiting", color: "bg-emerald-50 text-emerald-800 border-emerald-200" },
    3: { label: "Completed", color: "bg-purple-50 text-purple-800 border-purple-200" },
    4: { label: "Contaminated", color: "bg-red-50 text-red-800 border-red-200" },
  };
  return phases[phase] ?? { label: "Unknown", color: "bg-stone-100 text-stone-800 border-stone-200" };
};
                ...
                >
                  <div class="flex items-center justify-between">
                    <div>
                      ...
                      <p class="text-sm font-medium text-stone-500 uppercase tracking-wide">${batch.batchCode}</p>
                    </div>

                    <div class="text-right">
                      <p class="text-sm text-stone-600">
                        <span class="font-semibold text-stone-800">${batch.currentFlushes}</span> flushes
                      </p>
                      <p class="text-xs text-stone-400 mt-1">Inoculated ${formatRelative(batch.inoculatedAt)}</p>
                    </div>
                  </div>
