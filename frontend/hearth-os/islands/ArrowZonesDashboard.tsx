  ...
  useEffect(() => {
    ...
    const handleCreate = async () => {
      ...
      try {
        ...
        state.formError = err instanceof Error ? err.message : "Failed to create zone";
      } finally {
    ...
    const inlineForm = () => html`
      <div class="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 mb-6">
        <h3 class="text-sm font-bold text-stone-700 uppercase tracking-wider mb-4">New Zone</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ${ArrowFormField({
            ...
            children: html`
              <input
                ...
                @input="${(e: Event) => { state.formName = (e.target as HTMLInputElement).value; }}"
          ...
          ${ArrowFormField({
            ...
            children: html`
              <select
                @change="${(e: Event) => { state.formZoneType = Number((e.target as HTMLSelectElement).value); }}"
              ...
              >
                ${ZONE_TYPES.map((t, i) => html`<option value="${i}">${t}</option>`)}
              </select>
            `,
          })}
          ${ArrowFormField({
            ...
            children: html`
              <input
                ...
                @input="${(e: Event) => { state.formDescription = (e.target as HTMLInputElement).value; }}"
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            type="button"
            @click="${() => { state.showForm = false; state.formError = ''; }}"
    ...
    const zoneCard = (zone: ZoneSummaryDto) => html`
      ...
      >
        <div class="flex items-center justify-between mb-3">
          <h3 class="font-bold text-stone-800 text-sm truncate">${zone.name}</h3>
          <span class="${`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${zoneTypeColor(zone.zoneType)}`}">
            ${zoneTypeName(zone.zoneType)}
          </span>
        </div>
        <div class="text-xs text-stone-400 font-mono truncate">${zone.id}</div>
        ${zone.parentZoneId
          ? html`<p class="text-xs text-stone-500 mt-2">Parent: <span class="font-mono">${zone.parentZoneId}</span></p>`
          : html``}
      </a>
    `;
    ...
    const template = html`
      <div class="p-8 max-w-7xl mx-auto">
        <!-- Back link -->
        <div class="mb-8">
          <a href="/iot" class="text-emerald-600 hover:text-emerald-800 font-semibold mb-4 inline-block transition">
            &larr; Back to Devices
          </a>
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 class="text-3xl font-extrabold text-stone-800 tracking-tight">IoT Zones</h1>
              <p class="text-stone-500 mt-1">Group and locate your devices hierarchically across the farm.</p>
            </div>
            <button
              type="button"
        ...
        <!-- Loading -->
        ${() => state.loading
          ? html`<div class="text-center py-12 text-stone-400">Loading zones...</div>`
          : html``}
        ...
        ${() => {
          ...
          if (state.zones.length === 0) {
            return ArrowEmptyState({
              ...
              message: "Create your first zone to organize your sensor network across the farm.",
            });
          }
          return html`
