...
import { extractErrors, type FieldErrors, CultureSchema } from "../utils/schemas.ts";

const cultureTypes = [
  { value: 0, label: "Sourdough Starter", desc: "Wild yeast + lactobacillus for bread leavening" },
  { value: 1, label: "Kombucha SCOBY", desc: "Symbiotic colony of bacteria and yeast for tea fermentation" },
  { value: 2, label: "Milk Kefir Grains", desc: "Polysaccharide matrix of bacteria/yeast for milk fermentation" },
  { value: 3, label: "Other", desc: "Water kefir, tempeh, vinegar mother, etc." }
];
  ...
  useEffect(() => {
    ...
    const onSubmit = async (e: Event) => {
      ...
      try {
        ...
        showToast("success", "Culture created!", `"${state.name}" is now in your culture registry.`);
        closeModal();
      } catch (err: unknown) {
        showToast("error", "Failed to create culture", err instanceof Error ? err.message : "Unknown error");
      } finally {
    ...
    const inputClass = (field: string) => () => 
      `w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition ${(state.errors as any)[field] ? "border-red-400 bg-red-50" : "border-stone-300"}`;
              ...
              <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden animate-[scaleIn_0.2s_ease-out]">
                ...
                <div class="px-6 py-4 border-b border-stone-100 flex items-center justify-between bg-stone-50">
                  <div class="flex items-center gap-2">
                    ...
                    ${ArrowTooltip({
                      text: "Cultures are living organisms you maintain (starters, SCOBYs, kefir grains). Register them here to track feeding schedules and lineage.",
                      children: ArrowInfoIcon()
                    })}
                  </div>
                  <button @click="${closeModal}" class="text-stone-400 hover:text-stone-600 hover:bg-stone-200 rounded p-1 transition">✕</button>
                </div>
                ...
                <div class="p-6">
                  <form @submit="${onSubmit}" class="flex flex-col gap-4">
                    ...
                    ${ArrowFormField({
                      ...
                      children: html`
                        <select
                          ...
                          value="${() => state.type}"
                          @change="${(e: Event) => state.type = (e.target as HTMLSelectElement).value}"
                          disabled="${() => state.isSubmitting}"
                        >
                          ${cultureTypes.map(t => html`<option value="${t.value}">${t.label}</option>`)}
                        </select>
                        ${() => {
                          const selectedType = cultureTypes.find(t => t.value === Number(state.type));
                          return selectedType ? html`<p class="text-xs text-stone-400 mt-1">💡 ${selectedType.desc}</p>` : '';
                        }}
                      `
                    })}
                    ...
                    ${ArrowFormField({
                      ...
                      children: html`
                        <textarea
                          ...
                          value="${() => state.origin}"
                          @input="${(e: Event) => state.origin = (e.target as HTMLTextAreaElement).value}"
                          disabled="${() => state.isSubmitting}"
                        ></textarea>
                      `
                    })}
                    ...
                    <div class="flex justify-end gap-3 mt-2">
                      ...
                      >
                        ${() => state.isSubmitting ? "Creating..." : "Register Culture"}
                      </button>
                    </div>
                  </form>
                </div>
