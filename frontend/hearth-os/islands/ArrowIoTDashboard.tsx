  ...
  useEffect(() => {
    ...
    const handleRegister = async (e: Event) => {
      ...
      try {
        ...
        state.regError = err instanceof Error
          ? err.message
          : "Registration failed";
      } finally {
    ...
    const statusBadge = (status: number) => {
      ...
      return html`
        <span
          class="px-2.5 py-1 ${style.bg} ${style
            .text} rounded-full text-xs font-semibold"
        >${name}</span>
      `;
    };
    ...
    const deviceCard = (device: DeviceSummaryDto) =>
      html`
        ...
        >
          ...
          <p class="text-xs text-stone-400 font-mono mb-1">${device
            .deviceCode}</p>
                ...
                ${() =>
                  state.devices.length === 0
                    ...
                    : html`
                      ...
                      >
                        ${() =>
                          state.devices.map((device) => deviceCard(device))}
                      </div>
                    `}
              </div>
            `}
