...
export default function ArrowMushroomActionPanel(
  { batchId, currentPhase }: Props,
) {
  ...
  useEffect(() => {
      ...
      try {
        if (state.tempF) {
          await MushroomAPI.recordTemperature(batchId, {
            temperatureF: Number(state.tempF),
          });
        }
        if (state.humidity) {
          await MushroomAPI.recordHumidity(batchId, {
            humidityPercent: Number(state.humidity),
          });
        }
    ...
    const template = html`
      <div class="space-y-4">
        <!-- Tabs -->
        <div class="flex">
          <button class="${tabClass("environment")}" @click="${() =>
            state.activeTab = "environment"}">Environment</button>
          <button class="${tabClass("phase")}" @click="${() =>
            state.activeTab = "phase"}">Phase</button>
          <button
              ...
              <div class="space-y-4 pt-2">
                <div>
                  ...
                  <input
                    ...
                    value="${() => state.tempF}"
                    @input="${(e: Event) =>
                      state.tempF = (e.target as HTMLInputElement).value}"
                  />
                </div>
                <div>
                  <label class="block text-xs font-medium text-stone-700 mb-1"
                  ...
                  <input
                    ...
                    @input="${(e: Event) =>
                      state.humidity = (e.target as HTMLInputElement).value}"
                  />
                </div>
                <button
                  @click="${recordEnvironment}"
                  disabled="${() =>
                    state.isSubmitting || (!state.tempF && !state.humidity)}"
                  class="${btnClass}"
                >
                  Log Environment
                </button>
            ...
            ? html`
              <div class="space-y-3 pt-2">
                ...
                ${currentPhase === 0
                  ...
                  : ""}
                <button
                  class="w-full rounded-md bg-stone-100 text-stone-600 px-4 py-2 text-sm hover:bg-stone-200 transition"
                >
                  Mark Completed
                </button>
                <div class="pt-4 border-t border-stone-100">
                  <button
                    class="w-full rounded-md bg-red-50 text-red-700 border border-red-200 px-4 py-2 text-sm hover:bg-red-100 transition"
                  >
                    Mark Contaminated
                  </button>
                </div>
              ...
              <div class="space-y-4 pt-2">
                <div>
                  ...
                  <input
                    ...
                    @input="${(e: Event) =>
                      state.flushYield = (e.target as HTMLInputElement).value}"
                  />
                </div>
                <button
                  @click="${recordFlush}"
