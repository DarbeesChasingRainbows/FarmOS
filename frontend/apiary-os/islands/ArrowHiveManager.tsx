...
export default function ArrowHiveManager() {
  ...
  useEffect(() => {
    ...
    const mapHive = (h: HiveSummary): Hive => ({
      ...
      mites: h.miteCount || 0,
      health:
        h.status === "Attention" || (h.miteCount && h.miteCount > 3)
          ? "attention"
          : h.status === "Resting"
            ? "resting"
            : "active",
    ...
    const loadHives = async () => {
      try {
        ...
        state.hives = [
          { id: "h1", name: "Hive Alpha", location: "South Orchard", type: "Langstroth", queenStatus: "Present", lastInspection: "Mar 15, 2026", mites: 1.2, health: "active", honeySupers: 2 },
          { id: "h2", name: "Hive Bravo", location: "North Meadow", type: "Langstroth", queenStatus: "Present", lastInspection: "Mar 15, 2026", mites: 0.8, health: "active", honeySupers: 3 },
          { id: "h3", name: "Hive Charlie", location: "Garden Wall", type: "Top Bar", queenStatus: "Present", lastInspection: "Mar 10, 2026", mites: 3.1, health: "attention", honeySupers: 1 },
          { id: "h4", name: "Hive Delta", location: "South Orchard", type: "Langstroth", queenStatus: "Unknown", lastInspection: "Mar 5, 2026", mites: 2.4, health: "active", honeySupers: 2 },
          { id: "h5", name: "Hive Echo", location: "West Pasture", type: "Warr\u00E9", queenStatus: "Present", lastInspection: "Mar 15, 2026", mites: 0.5, health: "active", honeySupers: 1 },
          { id: "h6", name: "Hive Foxtrot", location: "Barn Shelter", type: "Langstroth", queenStatus: "Absent", lastInspection: "Mar 15, 2026", mites: 0, health: "resting", honeySupers: 0 },
        ];
      } finally {
        state.loading = false;
      }
    ...
    const handleCreateHive = async () => {
      ...
      try {
        ...
        showToast("success", "Hive registered!", result.data.name + " is now tracked.");
        state.showCreateModal = false;
        state.createName = "";
        state.createLocation = "";
        loadHives();
      } catch (err: unknown) {
        showToast("error", "Failed to create hive", err instanceof Error ? err.message : "Unknown error");
      } finally {
    ...
    const handleInspect = async () => {
      ...
      try {
        ...
        showToast("success", "Inspection recorded", hive.name + " inspection logged.");
        state.showInspectForm = false;
        state.inspectBrood = "";
        ...
        showToast("error", "Failed to record inspection", err instanceof Error ? err.message : "Unknown error");
      } finally {
    ...
    const handleTreat = async () => {
      ...
      try {
        ...
        showToast("success", "Treatment recorded", state.treatName + " applied to " + hive.name + ".");
        state.showTreatForm = false;
    ...
    const handleHarvest = async () => {
      ...
      try {
        ...
        showToast("success", "Harvest recorded!", "Honey harvested from " + hive.name + ".");
    ...
    // --- Filter Button ---
    const filterBtn = (label: string, filter: FilterType, count: () => number) => html`
      <button type="button"
        ...
        ${label}
        <span class="${() => state.filter === filter ? 'bg-amber-600' : 'bg-stone-200'} text-xs px-1.5 py-0.5 rounded-full font-bold">
          ${count}
        </span>
      </button>
    `;
    ...
    const hiveCard = (hive: Hive) => {
      ...
      return html`
        <button type="button"
          ...
          class="${() => state.selectedId === hive.id
            ? 'border-amber-400 ring-2 ring-amber-200/50'
            : 'border-stone-200/60 hover:border-amber-200'
          } bg-white rounded-2xl border shadow-sm p-5 hover:shadow-md transition-all text-left cursor-pointer w-full">
          <div class="flex items-start justify-between mb-3">
            <div>
              <h3 class="text-base font-bold text-stone-800">${hive.name}</h3>
              <p class="text-xs text-stone-400 mt-0.5">${hive.location} \u00B7 ${hive.type}</p>
            </div>
            ${ArrowStatusBadge({ variant: hive.health })}
          </div>
          <div class="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-stone-100">
            <div>
              <p class="text-[10px] text-stone-400 uppercase tracking-wider font-medium">Queen</p>
              <p class="text-sm font-semibold mt-0.5 ${queenColor(hive.queenStatus)}">${hive.queenStatus}</p>
            </div>
            <div>
              <p class="text-[10px] text-stone-400 uppercase tracking-wider font-medium">Mites/100</p>
              <p class="text-sm font-semibold mt-0.5 ${miteColor(hive.mites)}">${hive.mites}</p>
            </div>
          </div>
          <div class="mt-3 bg-stone-100 rounded-full h-1.5">
            <div class="${miteBarColor(hive.mites)} h-1.5 rounded-full transition-all" style="width: ${miteBarWidth}%"></div>
          </div>
          <div class="flex items-center justify-between mt-2">
            <span class="text-[10px] text-stone-400">${hive.honeySupers} supers</span>
            <span class="text-[10px] text-stone-400">${hive.lastInspection}</span>
          </div>
        </button>
      `;
    };
    ...
    const sidebarContent = () => {
      ...
      return html`
        <div class="p-6 h-full flex flex-col">
          ...
          <div class="flex items-start justify-between mb-6 pb-4 border-b border-stone-100">
            <div>
              ...
              <div class="flex items-center gap-2">
                ${ArrowStatusBadge({ variant: hive.health })}
                <span class="text-sm text-stone-500">${hive.location} \u00B7 ${hive.type}</span>
              </div>
            </div>
          ...
          <!-- Vitals Grid -->
          <div class="flex-1">
            <h4 class="text-xs font-bold text-stone-800 uppercase tracking-wider mb-3">Vital Statistics</h4>
            <div class="grid grid-cols-2 gap-3 mb-6">
              <div class="bg-stone-50 rounded-xl p-3 text-center border border-stone-100">
                <p class="text-xl font-bold ${queenColor(hive.queenStatus)}">${hive.queenStatus}</p>
                <p class="text-[10px] text-stone-500 uppercase tracking-wide mt-1">Queen Status</p>
              </div>
              <div class="bg-stone-50 rounded-xl p-3 text-center border border-stone-100">
                <p class="text-xl font-bold ${miteColor(hive.mites)}">${hive.mites}</p>
                <p class="text-[10px] text-stone-500 uppercase tracking-wide mt-1">Mites/100</p>
              </div>
              <div class="bg-stone-50 rounded-xl p-3 text-center border border-stone-100">
                <p class="text-xl font-bold text-stone-700">${hive.honeySupers}</p>
                <p class="text-[10px] text-stone-500 uppercase tracking-wide mt-1">Honey Supers</p>
              </div>
              <div class="bg-stone-50 rounded-xl p-3 text-center border border-stone-100">
                <p class="text-sm font-bold text-stone-700 mt-1">${hive.lastInspection}</p>
                <p class="text-[10px] text-stone-500 uppercase tracking-wide mt-1">Last Inspected</p>
              </div>
            </div>

            <!-- Inspection Form (collapsible) -->
            ${() => state.showInspectForm ? html`
                ...
                <div class="flex flex-col gap-3">
                  <div class="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="date" class="${inputCls("date", state.inspectErrors)}"
                        value="${state.inspectDate}"
                        @input="${(e: Event) => { state.inspectDate = (e.target as HTMLInputElement).value; }}" />
                    </div>
                    <div>
                      <label class="text-xs font-medium text-stone-700">Queen Seen</label>
                      <div class="flex gap-2 mt-1">
                        <button type="button" @click="${() => { state.inspectQueenSeen = true; }}"
                          class="${() => state.inspectQueenSeen ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-stone-100 text-stone-500'} px-3 py-1 text-xs rounded-lg font-medium border transition">Yes</button>
                        <button type="button" @click="${() => { state.inspectQueenSeen = false; }}"
                          class="${() => !state.inspectQueenSeen ? 'bg-red-100 text-red-800 border-red-300' : 'bg-stone-100 text-stone-500'} px-3 py-1 text-xs rounded-lg font-medium border transition">No</button>
                      </div>
                    </div>
                  </div>
                  <div class="grid grid-cols-2 gap-3">
                    <div>
                      <label class="text-xs font-medium text-stone-700">Brood Pattern *</label>
                      <select class="${inputCls("broodPattern", state.inspectErrors)}"
                        @change="${(e: Event) => { state.inspectBrood = (e.target as HTMLSelectElement).value; }}">
                    ...
                    <div>
                      ...
                      <select class="${inputCls("temperament", state.inspectErrors)}"
                        @change="${(e: Event) => { state.inspectTemperament = (e.target as HTMLSelectElement).value; }}">
                        <option value="">Select...</option>
                  ...
                  <div>
                    <label class="text-xs font-medium text-stone-700">Mites per 100 bees *</label>
                    <input type="number" step="0.1" class="${inputCls("mitesPerHundred", state.inspectErrors)}"
                      placeholder="e.g. 2.5"
                      @input="${(e: Event) => { state.inspectMites = (e.target as HTMLInputElement).value; }}" />
                  </div>
                  <div>
                    <label class="text-xs font-medium text-stone-700">Notes</label>
                    <textarea class="${inputCls("notes", state.inspectErrors)}" rows="2"
                      placeholder="Optional observations..."
                      @input="${(e: Event) => { state.inspectNotes = (e.target as HTMLTextAreaElement).value; }}"></textarea>
                  </div>
                  <div class="flex gap-2">
                    <button type="button" @click="${handleInspect}"
                      ...
                      ${() => state.inspectSubmitting ? "Saving..." : "Save Inspection"}
                    </button>
                    <button type="button" @click="${() => { state.showInspectForm = false; }}"
            ...
            ${() => state.showTreatForm ? html`
              <div class="mb-4 p-4 bg-red-50 rounded-xl border border-red-200">
                ...
                <div class="flex flex-col gap-3">
                  <div>
                    <label class="text-xs font-semibold text-stone-700 uppercase">Treatment</label>
                    <select class="w-full px-3 py-2 border border-stone-300 bg-white rounded-lg text-sm mt-1 focus:ring-2 focus:ring-amber-500 outline-none"
                      @change="${(e: Event) => { state.treatName = (e.target as HTMLSelectElement).value; }}">
                      <option value="">Select...</option>
                  ...
                  <div>
                    <label class="text-xs font-semibold text-stone-700 uppercase">Method</label>
                    <select class="w-full px-3 py-2 border border-stone-300 bg-white rounded-lg text-sm mt-1 focus:ring-2 focus:ring-amber-500 outline-none"
                      @change="${(e: Event) => { state.treatMethod = (e.target as HTMLSelectElement).value; }}">
                      <option value="">Select...</option>
          ...
          <div class="pt-4 border-t border-stone-100 mt-auto">
            <p class="text-[10px] text-stone-500 uppercase tracking-wider mb-2 font-semibold">Quick Actions</p>
            <div class="flex flex-col gap-2">
              ${() => !state.showInspectForm ? html`
                <button type="button" @click="${() => { state.showInspectForm = true; state.showTreatForm = false; }}"
              ...
              ` : html``}
              ${() => !state.showTreatForm ? html`
                <button type="button" @click="${() => { state.showTreatForm = true; state.showInspectForm = false; }}"
              ...
              ` : html``}
              <button type="button" @click="${() => { state.showHarvestConfirm = true; }}"
    ...
    const createModal = () => html`
      <div class="${() => state.showCreateModal ? 'fixed inset-0 bg-stone-900/50 backdrop-blur-sm flex items-center justify-center z-50 animate-[fadeIn_0.2s_ease-out]' : 'hidden'}">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden animate-[scaleIn_0.2s_ease-out]">
          <div class="px-6 py-4 border-b border-stone-100 flex items-center justify-between bg-stone-50">
            ...
            <button type="button" @click="${() => { state.showCreateModal = false; }}"
              class="text-stone-400 hover:text-stone-600 hover:bg-stone-200 rounded p-1 transition">\u2715</button>
          </div>
          <div class="p-6 flex flex-col gap-4">
            <div>
              <label class="text-sm font-medium text-stone-700">Hive Name *</label>
              <input type="text" class="${() => inputCls("name", state.createErrors)}"
                placeholder="e.g. Hive Alpha"
                @input="${(e: Event) => { state.createName = (e.target as HTMLInputElement).value; }}" />
              ${() => state.createErrors.name ? html`<p class="text-xs text-red-600 mt-1">${state.createErrors.name}</p>` : html``}
            </div>
            <div>
              <label class="text-sm font-medium text-stone-700">Location *</label>
              <input type="text" class="${() => inputCls("location", state.createErrors)}"
                placeholder="e.g. South Orchard"
                @input="${(e: Event) => { state.createLocation = (e.target as HTMLInputElement).value; }}" />
              ${() => state.createErrors.location ? html`<p class="text-xs text-red-600 mt-1">${state.createErrors.location}</p>` : html``}
            </div>
            <div>
              <label class="text-sm font-medium text-stone-700">Hive Type *</label>
              <select class="${() => inputCls("hiveType", state.createErrors)}"
                @change="${(e: Event) => { state.createType = (e.target as HTMLSelectElement).value; }}">
                <option value="0">Langstroth</option>
                <option value="1">Top Bar</option>
                <option value="2">Warr\u00E9</option>
              </select>
            </div>
            <div class="flex justify-end gap-3 mt-2">
              <button type="button" @click="${() => { state.showCreateModal = false; }}"
                class="px-4 py-2 rounded-lg font-medium text-stone-600 hover:bg-stone-100 transition">Cancel</button>
              <button type="button" @click="${handleCreateHive}"
    ...
    const template = html`
      <div class="px-6 py-8 max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h1 class="text-3xl font-extrabold text-stone-800 tracking-tight">Hives</h1>
            <p class="text-stone-500 mt-1">Click any hive to inspect, treat, or harvest.</p>
          </div>
          <button type="button" @click="${() => { state.showCreateModal = true; }}"
        ...
        <div class="flex flex-wrap gap-2 mb-6">
          ${filterBtn("All", "all", () => state.hives.length)}
          ${filterBtn("Active", "active", () => countByHealth("active"))}
          ${filterBtn("Attention", "attention", () => countByHealth("attention"))}
        ...
        ` : html`
          <div class="relative flex min-h-[500px]">
            <!-- Grid -->
            <div class="${() => state.sidebarOpen ? 'flex-1 mr-[420px] transition-all duration-300' : 'flex-1 transition-all duration-300'}">
              ${() => filteredHives().length === 0
                ? ArrowEmptyState({ icon: "\uD83D\uDC1D", title: "No hives match this filter", message: "Try a different filter or create a new hive." })
                : html`
            ...
            <!-- Sidebar -->
            <aside class="${() => state.sidebarOpen ? 'translate-x-0' : 'translate-x-full'} fixed top-0 right-0 h-full w-[400px] bg-white border-l border-stone-200 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out overflow-y-auto">
              ${sidebarContent}
            </aside>
          </div>
        `}
        ...
        ${ArrowConfirmDialog({
          ...
          message: () => {
            ...
            return "Apply " + state.treatName + " to " + (h ? h.name : "hive") + "?";
          },
          onConfirm: handleTreat,
          onCancel: () => { state.showTreatConfirm = false; },
          danger: true,
        })}
        ${ArrowConfirmDialog({
          ...
          onCancel: () => { state.showHarvestConfirm = false; },
        })}
      </div>
    `;
