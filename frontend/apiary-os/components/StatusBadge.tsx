...
export default function StatusBadge(
  { variant, label }: { variant: BadgeVariant; label?: string },
) {
  ...
  return (
    <span
      class={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold uppercase tracking-wider rounded-full border ${
        variants[variant]
      }`}
    >
      <span class="text-[10px]">{icons[variant]}</span>
      {displayLabel}
    </span>
  );
}
