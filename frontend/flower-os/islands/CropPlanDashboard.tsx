...
import type { CropPlanDetail, CropPlanSummary } from "../utils/farmos-client.ts";

const CHANNEL_LABELS = ["Farmers Market", "CSA", "Wholesale", "Wedding", "Direct Sale"];
      ...
      <div class="flex gap-6">
        ...
        <div class="w-80 flex-shrink-0">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-sm font-semibold text-stone-500 uppercase tracking-wide">Plans</h3>
            <button
          ...
          {showCreateForm.value && (
            <div class="mb-4 p-4 bg-white rounded-xl border border-stone-200 space-y-3">
              <input
                ...
                onInput={(e) => (newPlanName.value = (e.target as HTMLInputElement).value)}
              ...
              <div class="flex gap-2">
                <select
                  ...
                  onChange={(e) => (newSeason.value = (e.target as HTMLSelectElement).value)}
                ...
                <input
                  type="number"
                  value={newYear.value}
                  onInput={(e) => (newYear.value = +(e.target as HTMLInputElement).value)}
                  class="w-24 px-3 py-2 text-sm border border-stone-300 rounded-lg"
                />
              </div>
              <div class="flex gap-2">
                <button onClick={createPlan} class="flex-1 px-3 py-2 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                  Create
                </button>
          ...
          {loading.value
            ? <div class="text-center py-8 text-stone-400">Loading plans...</div>
            : plans.value.length === 0
            ? (
              <div class="text-center py-8 text-stone-400">
                <p class="text-lg">No crop plans yet</p>
                <p class="text-sm mt-1">Create a seasonal plan to track profitability.</p>
              </div>
            )
            : (
              <div class="space-y-2">
                {plans.value.map((plan) => {
                  ...
                  return (
                    ...
                    >
                      <div class="font-semibold text-stone-800">{plan.planName}</div>
                      <div class="text-sm text-stone-500">{plan.seasonName} {plan.seasonYear}</div>
                      <div class="flex justify-between mt-2 text-xs">
                        <span class="text-stone-400">{plan.bedCount} beds · {plan.totalStemsHarvested} stems</span>
                        <span class={profit >= 0 ? "text-emerald-600 font-medium" : "text-red-600 font-medium"}>
                          ${profit.toFixed(0)}
                        </span>
                      </div>
                    </button>
...
function PlanDetailView({ plan }: { plan: CropPlanDetail }) {
  ...
  return (
    <div class="bg-white rounded-xl border border-stone-200 p-6" style="animation: slideIn 0.2s ease-out">
      ...
      <p class="text-stone-500 text-sm mb-6">{plan.seasonName} {plan.seasonYear}</p>

      {/* Financial Summary */}
      <div class="grid grid-cols-4 gap-4 mb-6">
        <FinCard label="Revenue" value={`$${plan.totalRevenue.toFixed(0)}`} color="text-emerald-600" />
        <FinCard label="Costs" value={`$${plan.totalCosts.toFixed(0)}`} color="text-red-600" />
        <FinCard label="Profit" value={`$${profit.toFixed(0)}`} color={profit >= 0 ? "text-emerald-600" : "text-red-600"} />
        <FinCard label="Stems" value={plan.totalStemsHarvested.toString()} color="text-stone-800" />
      </div>

      {/* Bed Assignments */}
      {plan.bedAssignments.length > 0 && (
        <div class="mb-6">
          <h3 class="text-sm font-semibold text-stone-500 uppercase tracking-wide mb-3">Bed Assignments</h3>
          <div class="space-y-2">
            {plan.bedAssignments.map((a, i) => (
              <div key={i} class="flex items-center justify-between p-3 bg-stone-50 rounded-lg border border-stone-100">
                ...
                <span class="text-sm text-stone-500">{a.plannedSuccessions} successions</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {plan.revenues.length > 0 && (
        <div>
          <h3 class="text-sm font-semibold text-stone-500 uppercase tracking-wide mb-3">Revenue by Channel</h3>
          <div class="space-y-2">
            {plan.revenues.map((r, i) => (
              <div key={i} class="flex items-center justify-between p-3 bg-stone-50 rounded-lg border border-stone-100">
                <span class="text-stone-700">{CHANNEL_LABELS[r.channel] ?? "Unknown"}</span>
                <span class="font-semibold text-emerald-600">${r.amount.toFixed(2)}</span>
              </div>
            ))}
          </div>
        </div>
